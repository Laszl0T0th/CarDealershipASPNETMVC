...
// HU
// a migrációt meg kell csinálni előtte, így meg tudjuk változtatnia az alapbeállításokat az adatellenőrzésnél
builder.Services.AddIdentity<ApplicationUser, IdentityRole>(option =>
{
    ...
    // HU
    // Egyéni e-mail megerősítési idő
    ...
    // HU
    // MaxFailedAccessAttempts  A fiók zárolása előtt engedélyezett sikertelen bejelentkezési
    ...
    // DefaultLockoutTimeSpan  A fiók zárolásának időtartamát adja meg. Az alapértelmezett 5 perc.
...
// HU
//A beépített DataProtectorTokenProvider különböző típusú jogkivonatokat hozhat létre,
//például e-mail megerősítő tokent, jelszó-visszaállítási tokent.
...
// HU
// Csak az e-mail megerősítési token típusának a token élettartamát módosítja
...
// A válasz erre az Indítási osztály Startup.cs fájlban található. A következő kódsorral a ASP.NET Core az SQLEmployeeRepository
...
// és elérhető legyen az adott HTTP-kérés teljes hatókörében. Egy másik új HTTP-kéréshez az SQLEmployeeRepository osztály új példánya
// lesz megadva, és az a HTTP-kérés teljes hatókörében elérhető lesz.
// AddSingleton így ettől függ a "CarAccessoriesController"
...
// az asp.net alapvető függőséginjektálási tárolóval.
// Ez lehetővé teszi számunkra, hogy ennek az osztálynak egy példányát bármely vezérlőbe befecskendezzük az alkalmazásunk során.
...
// HU
// Megjegyzés: Ha az [AllowAnonymous] attribútumot a vezérlő szintjén alkalmazza, a rendszer figyelmen kívül
// hagyja az ugyanazon vezérlőműveletekre vonatkozó [Authorize] attribútumattribútumokat.
// Attribútum globális alkalmazása Az [Authorize] attribútum globális alkalmazásához az alkalmazás
// összes vezérlőjére és vezérlőműveletére módosítania kell a kódot az indítási osztály ConfigureServices metódusában.
...
//Az első paraméter a szabályzat neve, a második paraméter pedig maga a szabályzat
...
builder.Services.AddAuthorization(options =>
{
    ...
    // HU
    // Engedélyezéskezelő regisztrációja
    // Egyéni engedélyezési kezelőt regisztrálunk a Startup osztály ConfigureServices() metódusában
...
// HU
// Engedélyezéskezelő regisztrációja
// Egyéni engedélyezési kezelőt regisztrálunk az indítási osztály ConfigureServices() metódusában
...
//Schließlich die Verwendung einer benutzerdefinierten Richtlinie zum Schutz von Ressourcen wie Controller-Aktionsmethoden
// HU
//Végül az egyéni szabályzat használata az erőforrások, például a vezérlő műveleti módszereinek védelméhez
...
//    // HU
//    // Ha több szerepet szeretne belefoglalni a szabályzatba, egyszerűen válassza el őket vesszővel
...
if (!app.Environment.IsDevelopment())
{
    ...
    // HU
    // Az alapértelmezett HSTS-érték 30 nap. Előfordulhat, hogy ezt módosítani szeretné az éles forgatókönyvek esetében, lásd: https://aka.ms/aspnetcore-hsts.
    app.UseHsts();
}
